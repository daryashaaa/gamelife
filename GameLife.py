...
class GameLife:
    def __init__(self, screen_width=800, screen_height=600, cell_size=10, alive_color=(0, 255, 255),
        ...
        :param alive_color: RGB, например (255 ,255,255) для ячеек
        :param dead_color: Мертвые клиенты RGB
    ...
    def set_grid(self, value=None, grid=0):
        """
        Установить сразу всю сетку. Установите одно значение или случайный 0/1.
        Пример:
          set_grid(0) # все клетки мертвы
          set_grid(1) # все живы
          set_grid() # случайные клиентки
          set_grid(None) # так же сулчайное распределение
    ...
    def update_generation(self):
        """
        Проверьте состояние текущего поколения, подготовьте следующее поколение
        :return:
        """
